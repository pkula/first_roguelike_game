...
def items(coordinates_items, x, y):
    coordinates = []
    for i in coordinates_items:
        if i[0] == x and i[1] == y:
            continue
        coordinates = coordinates + [i]
    return coordinates
...
def get_inventory(coordinates_items, x, y, inventory):
    for i in coordinates_items:
        if i[0] == x and i[1] == y:
            random_item = inv.get_weapon_or_armor()
            inventory = inv.add_item_to_table(random_item, inventory)
    return inventory
